...
export default async (request, response) => {
    ...
    try {
        ...
        if (isStartCommand || !userData) {
            ...
            consconst step1Question = "*ШАГ 1 из 10: ИДЕНТИЧНОСТЬ.*\\n\\nЭто самое главное. Каждое твое действие — это **движение** к твоей новой, усовершенствованной личности. \\n\\nНапиши, каким человеком ты хочешь стать? (например: \"Здоровым и энергичным\", \"Продуктивным и организованным\", \"Человеком-Системой\")";
                ...
                switch (currentStep) {
                    ...
                    case 'STEP_3':
                       ...
                       confirmationMessage = `✅ Принято! Ты будешь выполнять привычку **после того, как:** *${textToSave}*.\n\nЭто создаст **триггер**, который невозможно будет пропустить.`;
                        nextQuestion = "*ШАГ 4 из 10: Для завершения цикла привычки (Подсказка -> Действие -> Награда) тебе нужно вознаграждение - сигнал, который необходим мозгу, чтобы запомнить действие.\n\nНапиши, чем ты себя немедленно наградишь после выполнения микро-привычки (например: \"одну минуту поскроллю соц сети\", \"послушаю хорошую музыку\", \"похвалю себя в зеркало\").";
                        break;
                    case 'STEP_4':
                        updatePayload = { habit_reward: textToSave, onboarding_state: 'STEP_5' };
                        confirmationMessage = `✅ Принято! Твоя Награда: *${textToSave}*.\n\nТеперь, когда ты выполнишь привычку, твой мозг **немедленно** получит дофаминовый сигнал. Это закрепит позитивную связь!`;
