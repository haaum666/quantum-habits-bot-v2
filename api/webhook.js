...
export default async (request, response) => {
    ...
    try {
        ...
        if (isStartCommand || !userData) {
            ...
            const welcomeMessage = `🧠 Привет, *${userFirstName}*. Ты в системе *Квантумных Привычек*.\n\nДаже самое длинное путешествие начинается с подготовки. Всё начинается с ясности — *чего ты действительно хочешь*?`;
            ...
            if (currentStep === 'COMPLETED') {
                ...
                switch (currentStep) {
                    case 'STEP_1':
                        ...
                        // ИСПРАВЛЕНО: Грамматически верное подтверждение
                        ...
                        nextQuestion = `*ШАГ 2 из 10: Действие и правило двух минут.*\n\nЛюбая новая привычка должна занимать *не более 120 секунд* в самом ее начале.\n\nНапиши, какую микро-привычку ты готов выполнять *ежедневно?* _Например:_ \_отжаться 1 раз_\, \_прочитать 1 страницу сложной книги_\, \_выпить стакан воды\_).`;
                        break;
                    case 'STEP_2':
                        ...
                        nextQuestion = `*ШАГ 3 из 10: Триггер.*\n\nЛучший способ начать что-то новое — *встроить* это во что-то уже существующее. Мы используем *связывание привычек Habit Stacking*.\n\nНапиши, какое ежедневное действие послужит *триггером* для твоей микро-привычки. \n\nНапример: \_выключение будильникак_\, \_чистка зубов_\, \_утренняя проверка соцсетей_\).`;
                        break;
                    case 'STEP_3':
                        updatePayload = { habit_link_action: textToSave, onboarding_state: 'STEP_4' };
                        confirmationMessage = `✅ Принято! Твой триггер зафиксирован: *${textToSave}*, привычки связаны!`;
                        // Обновленный текст Шага 4
                        nextQuestion = `*ШАГ 4 из 10: Награда.*\n\nЧтобы завершить цикл привычки: _подсказка -> действие -> награда_, мозгу нужен дофаминовый сигнал. \n\n**Вознаграждение** — это ключ к закреплению позитивной связи.\n\nНапиши, чем ты себя немедленно наградишь после выполнения микро-привычки? \n\nНапример: "одну минуту посижу в соцсетях", "послушаю хорошую музыку", "похвалю себя в зеркало").`;
                        break;
                    case 'STEP_4':
                        ...
                        nextQuestion = `*ШАГ 5 из 10: Прогресс.*\n\nЗакрепление привычки требует не только немедленной награды, но и визуализации промежуточных достижений.\n\nКогда ты нажимаешь *✅ Готово* (или пишешь \`/done\`):\n1. бот фиксирует твою успешную **серию** (_streak_).\n\nТвоя цель — не прерывать эту серию.\n\nНапиши *Прогресс*, чтобы подтвердить, что ты готов ежедневно фиксировать свои достижения.`;
                        break;
                    case 'STEP_5':
